...
for P = step:step:P_max            % Loop over P (number of data points)
  arrAlpha(i) = P/N;
  arrQ_ls(i,1) = Q_ls(P, N, augm, 10, n_D);
  arrQ_ls(i,2) = Q_ls(P, N, augm, 100, n_D);
  arrQ_ls(i,3) = Q_ls(P, N, augm, 1000, n_D);
  arrP_ls(i) = P_ls(P, N);
  fprintf("%.2f/%d\n", P/N, 4);
  i = i + 1;
end
...
scatter(arrAlpha,arrQ_ls(:,1),50,'o','MarkerEdgeColor','#00315E','DisplayName',sprintf('Q_{l.s.}(P,%d), n_{max} = %d', N, 10));
scatter(arrAlpha,arrQ_ls(:,2),50,'s','MarkerEdgeColor','#00315E','DisplayName',sprintf('Q_{l.s.}(P,%d), n_{max} = %d', N, 100));
scatter(arrAlpha,arrQ_ls(:,3),50,'x','MarkerEdgeColor','#00315E','DisplayName',sprintf('Q_{l.s.}(P,%d), n_{max} = %d', N, 1000));
...
title( { '{\bf\fontsize{20} Q_{l.s.}(P,N) vs. P_{l.s.}(P,N)}', sprintf('n_D = %d', n_D) },'FontWeight','Normal','FontSize',12);
lgd = legend;
lgd.FontSize = 12;
